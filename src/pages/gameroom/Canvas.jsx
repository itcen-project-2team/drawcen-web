...
const Canvas = ({ isQuizMaster, answer, gameId, turnInfo, timePercent }) => {
  ...
  // drawBackground 함수를 useCallback으로 감싸기
  ...
  // redrawCanvas 함수를 useCallback으로 감싸기
  ...
  useEffect(() => {
    ...
    const resizeCanvas = () => {
      ...
      // 기존 그림 복원 (history 기반으로)
      if (history.length > 0) {
        console.log('🎨 리사이즈 후 그림 복원:', history.length, '개 스트로크');
  ...
  }, [history, drawBackground, redrawCanvas]); // 필요한 의존성들 추가

  // 턴이 바뀔 때마다 캔버스 초기화
  useEffect(() => {
    if (!turnInfo?.turnId) return;
    
    console.log('새로운 턴 시작 - 캔버스 초기화 확인:', turnInfo.turnId);
    
    // 이전 턴 ID와 비교하여 실제로 새로운 턴일 때만 초기화
    if (prevTurnIdRef.current !== turnInfo.turnId) {
      console.log('🔄 실제 새로운 턴 - 캔버스 초기화 실행:', turnInfo.turnId);
      ...
      prevTurnIdRef.current = turnInfo.turnId;
    }
  }, [turnInfo?.turnId]);
